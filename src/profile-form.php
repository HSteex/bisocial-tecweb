...
if(!empty($_POST['email']) || !empty($_POST['nome']) ||
    ...
    $noUpdate = 0;                              //Variabile per non stampare se non modifico informazioni (senza contare l'immagine profilo)
...
<div class="container profile profile-view" id="profile">
    <div class="row">
        <?php
        if (isset($GLOBALS['updateResponse']) && !$noUpdate) {
        ...
        if (isset($GLOBALS['uploadResponse'])) {
            if (!$GLOBALS['uploadResponse']) {
                echo '<div class="col-md-12 alert-col relative">
                    <div id="image-upload" style="display: block; background-color: lightgreen" class="alert" role="alert"><button style="float: right" data-bs-dismiss="alert" aria-label="Chiudi" type="button" class="btn-close"></button><span id="profile-edit-message">' . $uploadMessage . '</span></div>
                </div>';
            } else {
                echo '<div class="col-md-12 alert-col relative">
                    <div id="image-upload" style="display: block; background-color: lightcoral" class="alert" role="alert"><button style="float: right" data-bs-dismiss="alert" aria-label="Chiudi" type="button" class="btn-close"></button><span id="profile-edit-message">' . $uploadMessage . '</span></div>
                </div>';
            }
    ...
    <form method="post" class="profile" name="profile-form" enctype="multipart/form-data">
        <div class="row profile-row" style="margin:20px 0px">
            ...
            <div class="col-md-8">
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-6">
                        <div class="form-group mb-3"><label class="form-label" style="font-family: 'Roboto Condensed', sans-serif;">Password </label><input class="form-control" type="password" name="password" id="password" autocomplete="off"></div>
                    </div>
                    <div class="col-sm-12 col-md-6">
